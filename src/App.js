...
const PlaylistReference = ({ playlists, isPlaylistsLoading }) => {
  return (
    <div className={`p-2 mb-8 max-h-full overflow-y-auto ${isPlaylistsLoading ? 'opacity-0' : 'opacity-100 transition-opacity duration-500 ease-in-out'}`}>
...
const Response = ({ promptResponse, isPromptLoading, recv }) => (
  <div className={`mb-8 ${!recv && !isPromptLoading ? 'opacity-0' : 'opacity-100 transition-opacity duration-500 ease-in-out'}`}>
